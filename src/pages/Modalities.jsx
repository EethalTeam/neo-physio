...
const FeesType = () => {
    ...
    const deleteModalities = async (id) => {
        try {
            ...
            toast({ title: "Deleted", description: "Modalities has been removed.", variant: "destructive" });
            getModalities();
            return response;
        } catch (error) {
            0
    ...
    const updateModalities = async (data) => {
        try {
            ...
            toast({ title: "Success", description: "Modalities updated successfully." });
            getModalities()
            setIsFormOpen(false)
            return response;
        } catch (error) {
        ...
        <div className="space-y-6">
                    ...
                    <CardContent>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b">
                                        <th className="text-left p-3 font-semibold text-gray-600">Modalities Name</th>
                                ...
                                <tbody>
                                    {modalities.map((mod) => (
                                        <tr key={mod._id} className="border-b hover:bg-gray-50/50 transition-colors">
                                            <td className="p-3 font-medium text-gray-800">{mod.modalitiesName}</td>
                                            {/* <td className="p-3">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${cat.ExpenseCategoryType === 'Income' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                          {cat.ExpenseCategoryType}
                                            ...
                                            <td className="p-3">
                                                <span className={`px-2 py-1 text-xs font-medium rounded-full ${mod.isActive ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle>{editingModalities ? 'Edit Modalities' : 'Add New Modalities'}</DialogTitle>
                        <DialogDescription>Define a new Modalities for tracking transactions.</DialogDescription>
                    </DialogHeader>
                    <form onSubmit={handleFormSubmit} className="space-y-6 pt-4">
                        <div className="space-y-2">
                            <Label htmlFor="modalitiesCode"> Modalities Code</Label>
                            <Input id="modalitiesCode" name="modalitiesCode" value={modalitiesForm.modalitiesCode} onChange={handleFormChange} required placeholder="e.g., MD001" />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="modalitiesName"> Modalities Name</Label>
                            <Input id="modalitiesName" name="modalitiesName" value={modalitiesForm.modalitiesName} onChange={handleFormChange} required placeholder="e.g., TENS" />
                        </div>
              ...
              <RadioGroup name="ExpenseCategoryType" value={categoryForm.ExpenseCategoryType} onValueChange={(val) => handleRadioChange('ExpenseCategoryType', val)} className="flex gap-4">
                ...
                <div className="flex items-center space-x-2"><RadioGroupItem value="Expense" id="type-expense" /><Label htmlFor="type-expense">Expense</Label></div>
                        ...
                        <div className="space-y-3">
                            <Label>Status</Label>
                            <RadioGroup name="isActive" value={modalitiesForm.isActive} onValueChange={(val) => handleRadioChange('isActive', val)} className="flex gap-4">
                        ...
                        <DialogFooter>
                            ...
                            <Button type="submit">{editingModalities ? 'Save Changes' : 'Add Modalities'}</Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            </Dialog>
