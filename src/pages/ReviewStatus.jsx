...
const ReviewStatus = () => {
  ...
  useEffect(()=>{
    ...
    try {
      const response = await apiRequest("ReviewStatus/deleteReviewStatus", {
        ...
        toast({ title: "Deleted", description: "ReviewStatus has been removed.", variant: "destructive" });
      getReviewStatus();
      return response;
    } catch (error) {0
      console.error('Error:', error);
    ...
    const createReviewStatus = async (data) => {
      try {
        const response = await apiRequest("ReviewStatus/createReviewStatus", {
         ...
         toast({ title: "Success", description: "ReviewStatus Create successfully." });
        getReviewStatus()
        setIsFormOpen(false)
        return response;
      } catch (error) {
 ...
 try {
      const response = await apiRequest("ReviewStatus/updateReviewStatus", {
        ...
        toast({ title: "Success", description: "ReviewStatus updated successfully." });
   getReviewStatus()
     setIsFormOpen(false)
      return response;
    } catch (error) {
  ...
  return (
    <div className="space-y-6 ">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex flex-col md:flex-row md:justify-between items-start gap-4">
        <div>
          <h1 className="md:text-3xl text-2xl font-bold text-gray-900 flex items-center gap-3"><Layers size={30} /> Review Status</h1>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }}>
        <Card className="medical-card hidden md:block">
          <CardHeader>
            <CardTitle className='text-lg md:text-2xl'>All  Review Status ({reviewStatus.length})</CardTitle>
            <CardDescription>List of all defined transaction  Review status.</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left p-3 font-semibold text-gray-600">Review Status Name</th>
                ...
                <tbody>
                  {reviewStatus.map((review) => (
                    <tr key={review._id} className="border-b hover:bg-gray-50/50 transition-colors">
                      <td className="p-3 font-medium text-gray-800">{review.reviewStatusName}</td>

                      <td className="p-3">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${review.isActive ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                      ...
                      <td className="p-3">
                        <div className="flex items-center justify-end gap-2">
                          {
                            Permissions.isEdit && 
                          <Button size="sm" variant="outline" onClick={() => handleEdit(review)}><Edit size={14} /></Button>
                        }
                        {
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {reviewStatus.map((review) => (
        ...
        >
          ...
          <div className="flex items-center justify-start gap-4 mb-3">
            ...
            <input type="color" value={review.reviewStatusTextColor || "#ffffff"} />
          </div>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingReviewStatus ? 'Edit ReviewStatus' : 'Add New ReviewStatus'}</DialogTitle>
            <DialogDescription>Define a new Review Status for tracking transactions.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleFormSubmit} className="space-y-6 pt-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="reviewStatusCode" name="reviewStatusCode" value={reviewStatusForm.reviewStatusCode} onChange={handleFormChange} required placeholder="e.g., RS001" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="reviewStatusName"> Review Status Name</Label>
              <Input id="reviewStatusName" name="reviewStatusName" value={reviewStatusForm.reviewStatusName} onChange={handleFormChange} required placeholder="e.g., compeleted" />
            </div>
            
             <div className="space-y-3">
              <Label>Status</Label>
              <RadioGroup name="isActive" value={reviewStatusForm.isActive} onValueChange={(val) => handleRadioChange('isActive', val)} className="flex gap-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit">{editingReviewStatus ? 'Save Changes' : 'Add Review Status'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
